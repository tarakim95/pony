...
### 2026-05-08 · 무료 단일 DB에서 마이그레이션 안전 절차
   무엇이 잘못됐나: HARD RULE 8 "dev 먼저"를 단일 DB 환경에 그대로 적용하려다 막힘.
   왜 그랬나: dev/prod 분리 가정으로 룰을 짰음.
   다음에 어떻게 막을지: 단일 DB일 땐 BEGIN/ROLLBACK 시뮬레이션 → COMMIT → 즉시 동작 테스트 → 롤백 SQL 준비 4종 세트로 대체.